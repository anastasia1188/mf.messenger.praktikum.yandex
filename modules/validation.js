...
function validateEMail() {
    return function(idElement, nameHiddenError) {
        ...
        if (!exports.REXP_EMAIL.test(email))
            errors.push(FORMAT_ERROR);
        if (email.search(exports.REXP_GAP) >= 0)
...
function validateLogin() {
    return function(idElement, nameHiddenError) {
        ...
        if (login.length < 5)
            errors.push(LENGTH_ERROR);
        if (!exports.REXP_LOGIN.test(login))
            errors.push(FORMAT_ERROR);
        if (login.search(exports.REXP_GAP) >= 0)
            errors.push(GAP_ERROR);
        if (errors.length > 0)
            result.isValid = false;
        result.errors = errors;
...
function validatePassword() {
    return function(idElement, nameHiddenError) {
        ...
        if (password.search(exports.REXP_LITERAL) < 0) {
            errors.push(LITERAL_ERROR);
        }
        if (password.search(exports.REXP_NUMERAL) < 0) {
            errors.push(NUMERAL_ERROR);
        }
        if (password.search(exports.REXP_GAP) >= 0) {
            errors.push(GAP_ERROR);
        }
        if (errors.length > 0)
            result.isValid = false;
...
function isValidValues(arrInputs, nameHiddenError) {
    var result = true;
    for (var i = 0; i < arrInputs.length; i++) {
        var funcValidate = arrInputs[i].func;
